...
const Events = () => {
  ...
  const transformedData: MintScheduleDto[] = [
    ...(Array.isArray(data?.mint_schedule)
      ? data.mint_schedule.map(transformMintSchedule)
      : []),
    ...(Array.isArray(data?.tournament)
      ? data.tournament.map(transformTournament)
      : []),
  ];

  const filteredDataByTime = transformedData.sort((a, b) => {
    const dateA =
      a.type === EventTypes.MINTS ? a.mint_start_date : a.start_time;
    const dateB =
      b.type === EventTypes.MINTS ? b.mint_start_date : b.start_time;
                    ...
                    <Grid item>
                      ...
                      >
                        ...
                        >
                          ...
                          <Typography
                            variant="subtitle2"
                            sx={{ fontWeight: 400, color: COLOR.baseSemiGray }}
                          >
