...
import { getAcademy } from '@/services/resources';
...
const Academy = () => {
  ...
  const { data, isLoading } = useQuery<AcademyDto[]>({
    queryKey: QueryKey.ACADEMY,
    queryFn: () => getAcademy(locale as string),
    enabled: pathname === '/academy',
  });

  const pinnedAcademies = React.useMemo(
    () => data?.filter((academy: AcademyDto) => academy.is_pinned === 1),
    [data]
  );
    ...
    <Layout>
      <Grid container justifyContent="center">
        <Grid item xl={9} lg={9} md={10} sm={12} xs={12}>
          <Stack spacing={3}>
            <Title
              title={t('resources.academy')}
              description={t('resources.academyDescription')}
            />
            {pinnedAcademies?.map((academy: AcademyDto) => (
              <MainCard
                key={academy.id}
                data={academy}
